...
export default function AdminUsersList({
  ...
  return (
    <div className="p-6 rounded-2xl border border-slate-900 bg-slate-950/40 backdrop-blur-md space-y-5 text-xs text-slate-350">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-850 focus:border-purple-500 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-100 placeholder-slate-650 outline-none transition"
          />
        </div>
                ...
                <tr key={item.id} className={`hover:bg-slate-900/10 transition ${isBlocked ? "opacity-60 bg-red-950/5" : ""}`}>
                  ...
                  <td className="py-3.5 font-sans">
                    <select
                      ...
                      className="bg-slate-950 border border-slate-850 rounded-lg px-2 py-1 text-slate-200 text-[10px] font-bold"
                    >
                  ...
                  <td className="py-3.5">
                    <span className={`px-2 py-0.5 text-[8px] font-bold rounded uppercase border ${item.role === "ADMIN" ? "bg-purple-950/40 border-purple-500/20 text-purple-400" : "bg-slate-900 border-slate-800 text-slate-450"}`}>
                      {item.role}
                    </span>
                  </td>
                  ...
                  <td className="py-3.5 pr-2 text-right">
                    <div className="flex items-center justify-end space-x-1.5">
                      ...
                      <button onClick={() => onToggleRole(item)} disabled={actionLoading === item.id} className="p-1.5 rounded-lg border bg-slate-900 border-slate-850 text-slate-450 hover:text-white transition">
      ...
      <div className="flex justify-between items-center pt-4 border-t border-slate-900 text-[9px] font-bold uppercase text-slate-500">
        ...
        <div className="flex space-x-2">
          <button onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page === 1} className="px-3 py-1.5 rounded bg-slate-900 border border-slate-850 text-slate-350 hover:text-white transition disabled:opacity-40">Prev</button>
          <button onClick={() => { if (page * perPage < filtered.length) setPage(page + 1); }} disabled={page * perPage >= filtered.length} className="px-3 py-1.5 rounded bg-slate-900 border border-slate-850 text-slate-350 hover:text-white transition disabled:opacity-40">Next</button>
        </div>
      </div>
    </div>
  );
