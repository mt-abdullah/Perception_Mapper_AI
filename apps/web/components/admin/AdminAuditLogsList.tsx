...
export default function AdminAuditLogsList({ logs, onExport }: AdminAuditLogsListProps) {
  ...
  return (
    <div className="p-6 rounded-2xl border border-slate-900 bg-slate-950/40 backdrop-blur-md space-y-4 text-xs text-slate-350">
      <div className="flex justify-between items-center pb-3 border-b border-slate-900">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-350">Platform Audit Logs</h3>
          ...
          <tbody className="divide-y divide-slate-900/60 font-mono text-[10px] text-slate-400">
            {filtered.map((log) => (
              <tr key={log.id} className="hover:bg-slate-900/10">
                ...
                <td className="py-2.5 font-sans text-slate-450 truncate max-w-[150px]" title={log.details}>{log.details}</td>
